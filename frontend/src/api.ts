...
export const api = {
  ...
  updateSupplierOrder(token: string, supplierOrderId: number, payload: { customerOrderId?: number | null; clearCustomerOrderId?: boolean; supplierId?: number }) {
    return request<{ status: string }>(
  ...
  updateSupplierOrderItems(token: string, supplierOrderId: number, payload: { items: PurchaseItem[]; currency: string }) {
    return request<{ status: string }>(
      `/supplier-orders/${supplierOrderId}/items`,
      {
        method: "PUT",
        body: JSON.stringify(payload)
      },
