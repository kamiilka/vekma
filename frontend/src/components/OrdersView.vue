...
// ── Edit order items (draft only) ────────────────────────────
const showEditOrderItems = ref(false);
const editItemsOrderId = ref(0);
const editItemsCurrency = ref<CurrencyCode>("UAH");
const editItemsList = ref<(PurchaseItem & { _name?: string })[]>([]);
const editItemsSaving = ref(false);
const editItemsError = ref("");
const editItemsLineSearch = ref("");
const editItemsLineProduct = ref<Product | null>(null);
const editItemsLineQty = ref(1);
const editItemsLinePrice = ref(0);
const editItemsShowDropdown = ref(false);
const allProductsForEdit = ref<Product[]>([]);

// Inline create new product from edit-order-items modal
const showEditItemsNewProduct = ref(false);
const editItemsNewProductForm = ref({ name: "", sku: "", barcode: "", category: "", brand: "", purchasePrice: 0, retailPrice: 0 });
const editItemsNewProductSaving = ref(false);
const editItemsNewProductError = ref("");

const filteredEditItemsProducts = computed(() => {
  const q = editItemsLineSearch.value.toLowerCase().trim();
  const list = allProductsForEdit.value;
...
async function openEditOrderItems() {
  const order = detailOrder.value as any;
  editItemsOrderId.value = order.id;
  editItemsCurrency.value = order.currency ?? "UAH";
  editItemsList.value = (order.items ?? []).map((i: any) => ({
    productId: i.productId,
    quantity: i.quantity,
    price: i.price,
    _name: i.productName || getProductName(i.productId),
  }));
  editItemsLineSearch.value = "";
  editItemsLineProduct.value = null;
  editItemsLineQty.value = 1;
  editItemsLinePrice.value = 0;
  editItemsError.value = "";
  showEditOrderItems.value = true;
  if (allProductsForEdit.value.length === 0) {
    try { allProductsForEdit.value = await api.products(props.session.token); } catch {}
  }
}

function selectEditItemsProduct(p: Product) {
  editItemsLineProduct.value = p;
  editItemsLineSearch.value = p.name;
  editItemsLinePrice.value = (p as any).purchasePrice || 0;
  editItemsShowDropdown.value = false;
}

function addEditItemsLine() {
  if (!editItemsLineProduct.value) return;
  const p = editItemsLineProduct.value;
  editItemsList.value.push({ productId: p.id, quantity: editItemsLineQty.value, price: editItemsLinePrice.value, _name: p.name });
  editItemsLineProduct.value = null;
  editItemsLineSearch.value = "";
  editItemsLineQty.value = 1;
  editItemsLinePrice.value = 0;
...
function hideEditItemsDropdown() {
  setTimeout(() => { editItemsShowDropdown.value = false; }, 150);
}

async function saveEditItemsNewProduct() {
  if (!editItemsNewProductForm.value.name.trim()) return;
  editItemsNewProductSaving.value = true;
  editItemsNewProductError.value = "";
  try {
    const created = await api.createProduct(props.session.token, {
      ...editItemsNewProductForm.value,
      currency: "UAH",
      stock: 0,
      minStock: 1,
    }) as any;
    allProductsForEdit.value = await api.products(props.session.token);
    // Auto-select the created product with its purchase price
    editItemsLineProduct.value = created;
    editItemsLineSearch.value = created.name;
    editItemsLinePrice.value = created.purchasePrice || 0;
    editItemsLineQty.value = 1;
    showEditItemsNewProduct.value = false;
    editItemsNewProductForm.value = { name: "", sku: "", barcode: "", category: "", brand: "", purchasePrice: 0, retailPrice: 0 };
  } catch (e: any) {
    if (e?.message === "SESSION_EXPIRED") emit("session-expired");
    else editItemsNewProductError.value = e?.message ?? "Помилка створення товару";
  } finally {
    editItemsNewProductSaving.value = false;
  }
}

async function saveEditOrderItems() {
  if (!editItemsList.value.length) { editItemsError.value = "Додайте хоча б одну позицію"; return; }
  editItemsSaving.value = true; editItemsError.value = "";
  try {
    await api.updateSupplierOrderItems(props.session.token, editItemsOrderId.value, {
      items: editItemsList.value.map(({ productId, quantity, price }) => ({ productId, quantity, price })),
      currency: editItemsCurrency.value,
    });
    // Refresh the order in detailOrder and supplierOrders list
    const refreshed = await api.supplierOrders(props.session.token);
    supplierOrders.value = refreshed as any;
    const updated = (refreshed as any[]).find((o: any) => o.id === editItemsOrderId.value);
    if (updated) detailOrder.value = updated;
    showEditOrderItems.value = false;
  } catch (e: any) {
    if (e?.message === "SESSION_EXPIRED") emit("session-expired");
    else editItemsError.value = e?.message ?? "Помилка збереження";
...
// ── Inline edit: linked customer order (supplier orders) ──────────
const editingLinkedCustomer = ref(false);
const editLinkedCustomerValue = ref<number | null>(null);
const editLinkedCustomerSaving = ref(false);
const editLinkedCustomerSearch = ref("");

const filteredLinkedCustomerOptions = computed(() => {
  const q = editLinkedCustomerSearch.value.trim().toLowerCase();
  if (!q) return customerOrders.value.slice(0, 8);
  return customerOrders.value.filter(o =>
    `#${o.id} ${o.customerName}`.toLowerCase().includes(q)
  ).slice(0, 8);
});

function startEditLinkedCustomer() {
  const order = detailOrder.value as any;
  editLinkedCustomerValue.value = order.customerOrderId ?? null;
  editLinkedCustomerSearch.value = order.customerOrderId
    ? `#${order.customerOrderId} ${customerOrders.value.find(o => o.id === order.customerOrderId)?.customerName ?? ""}`
    : "";
  editingLinkedCustomer.value = true;
}

async function saveLinkedCustomer() {
  editLinkedCustomerSaving.value = true;
  try {
    const order = detailOrder.value as any;
    const isClearing = editLinkedCustomerValue.value === null;
    await api.updateSupplierOrder(props.session.token, order.id, {
      ...(isClearing
        ? { clearCustomerOrderId: true }
        : { customerOrderId: editLinkedCustomerValue.value }),
      supplierId: order.supplierId,
    });
    (detailOrder.value as any).customerOrderId = editLinkedCustomerValue.value;
    const idx = supplierOrders.value.findIndex((o: any) => o.id === order.id);
    if (idx !== -1) (supplierOrders.value[idx] as any).customerOrderId = editLinkedCustomerValue.value;
    editingLinkedCustomer.value = false;
  } catch (e: any) {
    if (e?.message === "SESSION_EXPIRED") emit("session-expired");
  } finally {
    editLinkedCustomerSaving.value = false;
  }
}
...
// ── Close customer order (auto-pay + write-off stock) ─────────
const showCloseOrderModal = ref(false);
const closeOrderMethod = ref<PaymentMethod>("cash");
const closeOrderCashboxId = ref<number | null>(null);
const closeOrderNote = ref("");
const closeOrderSaving = ref(false);
const closeOrderError = ref("");

async function openCloseOrderModal() {
  closeOrderMethod.value = "cash";
  closeOrderCashboxId.value = null;
  closeOrderNote.value = "";
  closeOrderError.value = "";
  // Refresh the order to get up-to-date debt/paid values
  try {
    await loadCustomerOrders();
    const fresh = customerOrders.value.find(o => o.id === (detailOrder.value as any)?.id);
    if (fresh) detailOrder.value = fresh;
  } catch {}
...
async function doCloseOrder() {
  const order = detailOrder.value as any;
  if (!order) return;
  closeOrderSaving.value = true;
  closeOrderError.value = "";
  const stockWarnings: string[] = [];
  try {
    const debt: number = Math.max(0, (order.total ?? 0) - (order.paid ?? 0));

    // 1. Якщо є борг — оплачуємо залишок
    if (debt > 0) {
      await api.createPayment(props.session.token, {
        orderId: order.id,
        amount: debt,
        currency: order.currency,
        method: closeOrderMethod.value,
        cashboxId: closeOrderCashboxId.value ?? undefined,
        note: closeOrderNote.value || `Закриття замовлення #${order.id}`,
      });
    }

    // 2. Списуємо товари зі складу (write_off). Якщо товару бракує —
    //    фіксуємо попередження і продовжуємо, щоб не блокувати закриття.
    const items: Array<{ productId: number; productName?: string; quantity: number }> = order.items ?? [];
    for (const item of items) {
      if (item.productId > 0 && item.quantity > 0) {
        try {
          await api.createMovement(props.session.token, {
            productId: item.productId,
            warehouseId: 0,
            type: "write_off",
            quantity: item.quantity,
            note: closeOrderNote.value || `Видача по замовленню клієнта #${order.id}`,
          });
        } catch (movErr: any) {
          const name = item.productName || getProductName(item.productId);
          stockWarnings.push(`«${name}»: недостатньо на складі, списання пропущено`);
        }
      }
    }

    // 3. Переводимо у статус "closed"
    await api.updateOrderStatus(props.session.token, order.id, "closed" as CustomerOrderStatus);
    ...
    if (e?.message === "SESSION_EXPIRED") emit("session-expired");
    else closeOrderError.value = e?.message ?? "Помилка при закритті замовлення";
  } finally {
    closeOrderSaving.value = false;
  }
}
...
const linkedCustomerOrderId = ref<number | null>(null);
...
async function openCreateModal() {
  createError.value = "";
  linkedCustomerOrderId.value = null;
...
async function openCreateModalLinked(customerOrderId: number) {
  await openCreateModal();
  linkedCustomerOrderId.value = customerOrderId;
  activeSubTab.value = "supplier";
  // Pre-populate items from the customer order
  const co = customerOrders.value.find(o => o.id === customerOrderId);
  if (co && (co as any).items?.length) {
    orderItems.value = (co as any).items.map((item: any) => ({
      productId: item.productId,
      quantity: item.quantity,
      price: item.price ?? 0,
    }));
...
async function submitCreateOrder() {
  ...
  try {
    if (activeSubTab.value === "customer") {
      ...
      const newSupplierOrder = await api.createSupplierOrder(props.session.token, {
        supplierId: supplierForm.value.supplierId,
        currency: supplierForm.value.currency,
        items: orderItems.value.filter(i => i.productId > 0),
      });
      if (linkedCustomerOrderId.value) {
        await api.updateSupplierOrder(props.session.token, (newSupplierOrder as any).id, {
          customerOrderId: linkedCustomerOrderId.value,
        });
      }
      await loadSupplierOrders();
    } else if (activeSubTab.value === "repair") {
...
async function submitCreateOrderAndLinkSupplier() {
  createError.value = "";
  if (!customerForm.value.customerName.trim()) {
    createError.value = "Вкажіть ім'я клієнта";
    return;
  ...
  createLoading.value = true;
  try {
    const newOrder = await api.createOrder(props.session.token, {
      customerName: customerForm.value.customerName.trim(),
      items: orderItems.value.filter(i => i.productId > 0),
      reserve: customerForm.value.reserve,
      dueDate: customerForm.value.dueDate || undefined,
    ...
    showCreateModal.value = false;
    await nextTick();
    await openCreateModalLinked((newOrder as any).id);
  } catch (e: any) {
    if (e?.message === "SESSION_EXPIRED") emit("session-expired");
    else createError.value = e?.message ?? "Помилка створення замовлення";
  } finally {
...
<template>
  <main class="page-content">
        ...
        <div class="modal-box" style="max-width:600px;width:100%;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;border-bottom:1px solid var(--border);padding-bottom:0.75rem">
            ...
            <div style="display:flex;gap:0.4rem;align-items:center">
              <button
                v-if="detailType === 'supplier' && (detailOrder as any).status === 'draft'"
                class="ghost-button"
                style="font-size:0.8rem;padding:0.2rem 0.6rem;border-color:rgba(96,192,224,0.4);color:#60c0e0"
                @click="openEditOrderItems">✏️ Редагувати</button>
          ...
          <template v-if="detailType === 'customer'">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;font-size:0.88rem;margin-bottom:1rem">
              ...
              <div><span class="subtle">Борг:</span>
                <span :style="Math.max(0, ((detailOrder as any).total ?? 0) - ((detailOrder as any).paid ?? 0)) > 0 ? 'color:var(--danger);font-weight:700' : 'color:#6dd4a0;font-weight:700'">
                  {{ formatMoney(Math.max(0, ((detailOrder as any).total ?? 0) - ((detailOrder as any).paid ?? 0)), (detailOrder as any).currency) }}
                </span>
              </div>
              <div style="display:flex;align-items:center;gap:0.4rem;flex-wrap:wrap">
            ...
            <div style="border-top:1px solid var(--border);padding-top:0.75rem;margin-bottom:1rem">
              ...
              <button
                class="ghost-button"
                style="font-size:0.82rem;margin-top:0.5rem;padding:0.3rem 0.7rem;border:1px dashed rgba(96,192,224,0.4);border-radius:6px;color:#60c0e0;cursor:pointer;width:100%;text-align:center"
                @click="showDetailModal=false; $nextTick(() => openCreateModalLinked((detailOrder as any).id))">
            ...
            <div style="margin-top:1rem;padding-top:0.75rem;border-top:1px solid var(--border)">
              ...
              <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap">
                <select class="input" style="font-size:0.85rem;flex:1;min-width:140px" :value="(detailOrder as any).status"
                  @change="updateCustomerOrderStatus((detailOrder as any).id, ($event.target as HTMLSelectElement).value as CustomerOrderStatus)">
                  <option v-for="(label, val) in customerStatusLabel" :key="val" :value="val">{{ label }}</option>
                </select>
                <button
                  v-if="!['closed','cancelled'].includes((detailOrder as any).status)"
                  style="font-size:0.82rem;padding:0.35rem 0.9rem;background:rgba(109,212,160,0.15);border:1px solid rgba(109,212,160,0.45);color:#6dd4a0;border-radius:var(--radius-sm);cursor:pointer;font-weight:600;white-space:nowrap"
            ...
            <!-- Modal: Close order confirmation -->
            <div v-if="showCloseOrderModal" style="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:10000;display:flex;align-items:center;justify-content:center;padding:1rem" @click.self="showCloseOrderModal=false">
              <div style="background:var(--panel-bg);border:1px solid rgba(109,212,160,0.3);border-radius:12px;padding:1.5rem;max-width:420px;width:100%;box-shadow:0 16px 48px rgba(0,0,0,0.5)">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
                  <span style="font-size:1rem;font-weight:700">🔒 Закрити замовлення #{{ (detailOrder as any).id }}</span>
                  <button class="ghost-button" style="font-size:1rem;padding:0.1rem 0.4rem" @click="showCloseOrderModal=false">✕</button>
                </div>
                <p v-if="closeOrderError" style="color:var(--danger);font-size:0.82rem;margin:0 0 0.6rem">{{ closeOrderError }}</p>
                <div style="background:rgba(109,212,160,0.07);border:1px solid rgba(109,212,160,0.2);border-radius:8px;padding:0.7rem 0.9rem;margin-bottom:1rem;font-size:0.85rem">
                  <div style="margin-bottom:0.3rem">
                    <span style="color:var(--text-muted)">Сума замовлення:</span>
                    <strong style="margin-left:0.4rem">{{ formatMoney((detailOrder as any).total, (detailOrder as any).currency) }}</strong>
                  </div>
                  <div style="margin-bottom:0.3rem">
                    <span style="color:var(--text-muted)">Оплачено:</span>
                    <span style="margin-left:0.4rem;color:#6dd4a0">{{ formatMoney((detailOrder as any).paid ?? 0, (detailOrder as any).currency) }}</span>
                  </div>
                  <div>
                    <span style="color:var(--text-muted)">До сплати:</span>
                    <strong style="margin-left:0.4rem" :style="Math.max(0, ((detailOrder as any).total ?? 0) - ((detailOrder as any).paid ?? 0)) > 0 ? 'color:#ff9ca0' : 'color:#6dd4a0'">
                      {{ formatMoney(Math.max(0, ((detailOrder as any).total ?? 0) - ((detailOrder as any).paid ?? 0)), (detailOrder as any).currency) }}
                    </strong>
                  </div>
                </div>
                <p style="font-size:0.82rem;color:var(--text-muted);margin:0 0 0.9rem">
                  При закритті буде автоматично:
                  <br>• оплачено залишок боргу (якщо є)
                  <br>• списано товари зі складу
                  <br>• статус змінено на «Закрито»
                </p>
                <div v-if="Math.max(0, ((detailOrder as any).total ?? 0) - ((detailOrder as any).paid ?? 0)) > 0" style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-bottom:0.6rem">
                  <label style="font-size:0.78rem">Метод оплати
                    <select v-model="closeOrderMethod" class="input" style="margin-top:0.2rem">
                      <option value="cash">Готівка</option>
                      <option value="card">Картка</option>
                      <option value="bank">Банк</option>
                      <option value="virtual">Інше</option>
                  ...
                  <label style="font-size:0.78rem">Каса
                    <select v-model="closeOrderCashboxId" class="input" style="margin-top:0.2rem">
                      <option :value="null">— без каси —</option>
                      <option v-for="cb in allCashboxes" :key="cb.id" :value="cb.id">{{ cb.name }}</option>
                    </select>
                  </label>
                </div>
                <label style="font-size:0.78rem;display:block;margin-bottom:0.9rem">Примітка
                  <input v-model="closeOrderNote" class="input" style="margin-top:0.2rem;width:100%;box-sizing:border-box" placeholder="Необов'язково">
                </label>
                <div style="display:flex;gap:0.5rem;justify-content:flex-end">
                  <button class="ghost-button" style="font-size:0.82rem" @click="showCloseOrderModal=false" :disabled="closeOrderSaving">Скасувати</button>
                  <button style="background:rgba(109,212,160,0.2);border:1px solid rgba(109,212,160,0.5);color:#6dd4a0;border-radius:var(--radius-sm);padding:0.4rem 1.1rem;font-size:0.85rem;cursor:pointer;font-weight:700" @click="doCloseOrder" :disabled="closeOrderSaving">
                    {{ closeOrderSaving ? 'Збереження...' : '🔒 Підтвердити закриття' }}
                  </button>
                </div>
              </div>
            </div>
          ...
          <template v-else-if="detailType === 'supplier'">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;font-size:0.88rem;margin-bottom:1rem">
              ...
              <div><span class="subtle">Дата створення:</span> {{ formatDate((detailOrder as any).createdAt) }}</div>
              <div style="grid-column:1/-1;display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap">
                <span class="subtle">Замовлення клієнта:</span>
                <template v-if="!editingLinkedCustomer">
                  <span v-if="(detailOrder as any).customerOrderId"
                  ...
                  <span v-else class="subtle" style="font-size:0.82rem">не прив'язано</span>
                  <button class="ghost-button" style="font-size:0.72rem;padding:0.1rem 0.4rem" @click="startEditLinkedCustomer">✏</button>
                </template>
                <template v-else>
                  <div style="display:flex;flex-direction:column;gap:0.3rem;flex:1;min-width:200px">
                    <input
                      v-model="editLinkedCustomerSearch"
                      class="input"
                      style="font-size:0.85rem;padding:0.25rem 0.5rem"
                      placeholder="Пошук замовлення клієнта..."
                      @input="editLinkedCustomerValue = null"
                    />
                    <div v-if="editLinkedCustomerSearch && !editLinkedCustomerValue"
                      style="background:var(--surface);border:1px solid var(--border);border-radius:6px;max-height:160px;overflow-y:auto;z-index:10">
                      <div
                        v-for="co in filteredLinkedCustomerOptions" :key="co.id"
                        style="padding:0.35rem 0.6rem;cursor:pointer;font-size:0.83rem;display:flex;gap:0.5rem;align-items:center"
                        @mousedown.prevent="editLinkedCustomerValue = co.id; editLinkedCustomerSearch = `#${co.id} ${co.customerName}`">
                        <span style="color:#7ab99a;font-weight:600">#{{ co.id }}</span>
                        <span>{{ co.customerName }}</span>
                        <span class="subtle" style="font-size:0.75rem;margin-left:auto">{{ co.status }}</span>
                      </div>
                    </div>
                  </div>
                  <button
                    style="font-size:0.78rem;padding:0.2rem 0.6rem;background:#7ab99a;color:#fff;border:none;border-radius:var(--radius-sm);cursor:pointer;align-self:flex-start"
                    :disabled="editLinkedCustomerSaving"
                    @click="saveLinkedCustomer">{{ editLinkedCustomerSaving ? '...' : '✓' }}</button>
                  <button
                    v-if="(detailOrder as any).customerOrderId"
                    style="font-size:0.78rem;padding:0.2rem 0.6rem;background:rgba(220,80,80,0.15);color:#e07070;border:1px solid rgba(220,80,80,0.3);border-radius:var(--radius-sm);cursor:pointer;align-self:flex-start"
                    :disabled="editLinkedCustomerSaving"
                    @click="editLinkedCustomerValue = null; editLinkedCustomerSearch = ''; saveLinkedCustomer()">відв'язати</button>
                  <button class="ghost-button" style="font-size:0.78rem;padding:0.2rem 0.5rem;align-self:flex-start" @click="editingLinkedCustomer=false">✕</button>
                </template>
              </div>
            </div>
        ...
        <div class="modal-box" style="max-width:560px;width:100%;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;border-bottom:1px solid var(--border);padding-bottom:0.75rem">
            <h3 style="margin:0;font-size:1rem">
              ...
              <span v-if="linkedCustomerOrderId && activeSubTab === 'supplier'"
                style="font-size:0.75rem;font-weight:400;margin-left:0.5rem;padding:0.15rem 0.5rem;background:rgba(122,185,154,0.15);border:1px solid rgba(122,185,154,0.3);border-radius:5px;color:#7ab99a">
                🔗 до замовлення клієнта #{{ linkedCustomerOrderId }}
              </span>
            </h3>
          ...
          <div style="display:flex;justify-content:flex-end;gap:0.5rem;margin-top:1.25rem;padding-top:0.75rem;border-top:1px solid var(--border)">
            ...
            <button
              v-if="activeSubTab === 'customer'"
              class="ghost-button"
              style="font-size:0.82rem;padding:0.4rem 0.8rem;border:1px dashed rgba(96,192,224,0.5);border-radius:var(--radius-sm);color:#60c0e0;cursor:pointer"
              @click="submitCreateOrderAndLinkSupplier"
              :disabled="createLoading">
              {{ createLoading ? "Збереження..." : "+ Створити замовлення постачальнику" }}
            </button>
            <button
    ...
    <!-- ═══ МОДАЛЬНЕ ВІКНО РЕДАГУВАННЯ ПОЗИЦІЙ ЗАМОВЛЕННЯ ════ -->
    <teleport to="body">
      <div v-if="showEditOrderItems" class="modal-backdrop" @click.self="showEditOrderItems=false">
        <div class="modal-box" style="max-width:580px;width:100%">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;border-bottom:1px solid var(--border);padding-bottom:0.75rem">
            <h3 style="margin:0;font-size:1rem">✏️ Редагування замовлення #{{ editItemsOrderId }}</h3>
            <button class="ghost-button" style="font-size:1.2rem;padding:0 0.4rem;line-height:1" @click="showEditOrderItems=false">✕</button>
          </div>

          <p class="subtle" style="font-size:0.82rem;margin-bottom:0.9rem">
            Редагування позицій доступне лише для замовлень у статусі <strong>Чернетка</strong>.
          </p>

          <!-- Currency selector -->
          <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.9rem">
            <span style="font-size:0.85rem;color:var(--text-muted)">Валюта:</span>
            <select v-model="editItemsCurrency" class="input" style="width:120px;font-size:0.85rem;padding:0.25rem 0.5rem">
          ...
          <!-- Items table -->
          <div style="font-size:0.82rem;font-weight:600;color:#60c0e0;margin-bottom:0.5rem">📦 Позиції замовлення</div>
          <table v-if="editItemsList.length" style="font-size:0.84rem;margin-bottom:0.75rem;width:100%">
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="(item, i) in editItemsList" :key="i">
                <td style="padding:0.2rem 0.4rem 0.2rem 0">{{ item._name || `#${item.productId}` }}</td>
                <td style="text-align:right">
                  <input type="number" min="1" v-model.number="item.quantity"
                    style="width:60px;padding:0.15rem 0.3rem;text-align:right;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit">
                </td>
                <td style="text-align:right">
                  <input type="number" min="0" step="0.01" v-model.number="item.price"
                    style="width:75px;padding:0.15rem 0.3rem;text-align:right;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit">
                </td>
                <td style="text-align:right;font-weight:600">{{ (item.quantity * item.price).toFixed(2) }}</td>
                <td style="text-align:center">
                  <button class="ghost-button" style="padding:0.15rem 0.4rem;color:#ff9ca0;font-size:0.8rem" @click="editItemsList.splice(i,1)">✕</button>
                </td>
              </tr>
              <tr style="border-top:1px solid var(--border)">
                <td colspan="3" style="text-align:right;font-size:0.8rem;color:var(--text-muted);padding-top:0.3rem">Разом:</td>
                <td style="text-align:right;font-weight:700;color:#9fe8c4;padding-top:0.3rem">
                  {{ editItemsList.reduce((s, i) => s + i.quantity * i.price, 0).toFixed(2) }} {{ editItemsCurrency }}
                </td>
                <td></td>
              </tr>
            </tbody>
          </table>
          <div v-else class="subtle" style="font-size:0.85rem;margin-bottom:0.75rem">Немає позицій — додайте товар нижче</div>

          <!-- Add line -->
          <div style="border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:0.7rem;background:rgba(255,255,255,0.03);margin-bottom:1rem">
            <div style="font-size:0.8rem;color:var(--text-muted);margin-bottom:0.4rem">Додати позицію</div>
            <div style="position:relative;margin-bottom:0.4rem">
              <input
                v-model="editItemsLineSearch"
                @focus="editItemsShowDropdown=true"
                @blur="hideEditItemsDropdown()"
                placeholder="Пошук за назвою, SKU, штрихкодом..."
                style="width:100%;box-sizing:border-box;padding:0.3rem 0.5rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem"
              >
              <div v-if="editItemsShowDropdown && !editItemsLineProduct"
                style="position:absolute;top:100%;left:0;right:0;background:var(--panel-bg,#1a2030);border:1px solid rgba(96,192,224,0.4);border-radius:6px;z-index:9999;max-height:200px;overflow-y:auto;box-shadow:0 8px 24px rgba(0,0,0,0.45)">
                <div v-for="p in filteredEditItemsProducts" :key="p.id"
                  @mousedown.prevent="selectEditItemsProduct(p)"
                  style="padding:0.4rem 0.6rem;cursor:pointer;font-size:0.84rem;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.05)"
                  @mouseover="($event.currentTarget as HTMLElement).style.background='rgba(96,192,224,0.1)'"
                  @mouseleave="($event.currentTarget as HTMLElement).style.background=''">
                  <span>{{ p.name }}</span>
                  <span style="color:var(--text-muted);font-size:0.78rem">{{ p.sku }}</span>
                </div>
                <div v-if="!filteredEditItemsProducts.length" style="padding:0.5rem 0.7rem;font-size:0.84rem;color:var(--text-muted)">Не знайдено</div>
                <div
                  @mousedown.prevent="showEditItemsNewProduct=true; editItemsShowDropdown=false"
                  style="padding:0.4rem 0.7rem;cursor:pointer;font-size:0.84rem;color:#9fe8c4;border-top:1px solid rgba(255,255,255,0.08);font-weight:600"
                  @mouseover="($event.currentTarget as HTMLElement).style.background='rgba(159,232,196,0.08)'"
                  @mouseleave="($event.currentTarget as HTMLElement).style.background=''">
                  + Створити новий товар
                </div>
              </div>
            </div>
            <div v-if="editItemsLineProduct" style="display:flex;gap:0.4rem;align-items:center;flex-wrap:wrap">
              <span style="font-size:0.84rem;flex:1;min-width:120px;color:#9fe8c4">✓ {{ editItemsLineProduct.name }}</span>
              <input type="number" v-model.number="editItemsLineQty" placeholder="К-сть" min="1"
                style="width:65px;padding:0.25rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem;text-align:center">
              <input type="number" v-model.number="editItemsLinePrice" placeholder="Ціна" min="0"
                style="width:80px;padding:0.25rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem;text-align:center">
              <button class="ghost-button" style="padding:0.25rem 0.6rem;font-size:0.85rem" @click="addEditItemsLine">+ Додати</button>
              <button class="ghost-button" style="padding:0.25rem 0.4rem;font-size:0.8rem" @click="editItemsLineProduct=null;editItemsLineSearch=''">✕</button>
            </div>
            <div v-else style="margin-top:0.35rem">
              <button class="ghost-button" style="font-size:0.8rem;padding:0.2rem 0.5rem;color:#9fe8c4;border-color:rgba(159,232,196,0.3)"
                @click="showEditItemsNewProduct=true; editItemsShowDropdown=false">
                + Створити новий товар
              </button>
            </div>
          </div>

          <!-- Inline: форма створення нового товару -->
          <div v-if="showEditItemsNewProduct"
            style="border:1px solid rgba(159,232,196,0.25);border-radius:8px;padding:0.85rem;background:rgba(159,232,196,0.05);margin-bottom:1rem">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.65rem">
              <span style="font-size:0.85rem;font-weight:600;color:#9fe8c4">📦 Новий товар</span>
              <button class="ghost-button" style="font-size:0.8rem;padding:0.1rem 0.4rem" @click="showEditItemsNewProduct=false; editItemsNewProductError=''">✕</button>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-bottom:0.5rem">
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                Назва *
                <input v-model="editItemsNewProductForm.name" placeholder="Назва товару"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                SKU / Артикул
                <input v-model="editItemsNewProductForm.sku" placeholder="SKU"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                Штрихкод
                <input v-model="editItemsNewProductForm.barcode" placeholder="Штрихкод"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                Категорія
                <input v-model="editItemsNewProductForm.category" placeholder="Категорія"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                Закупівельна ціна
                <input type="number" min="0" step="0.01" v-model.number="editItemsNewProductForm.purchasePrice"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
              <label style="font-size:0.8rem;display:flex;flex-direction:column;gap:0.15rem">
                Роздрібна ціна
                <input type="number" min="0" step="0.01" v-model.number="editItemsNewProductForm.retailPrice"
                  style="padding:0.28rem 0.45rem;background:var(--bg-input,#1e2230);border:1px solid var(--border);border-radius:4px;color:inherit;font-size:0.84rem">
              </label>
            </div>
            <p v-if="editItemsNewProductError" class="error-text" style="font-size:0.8rem;margin-bottom:0.4rem">{{ editItemsNewProductError }}</p>
            <div style="display:flex;gap:0.5rem">
              <button
                style="font-size:0.82rem;padding:0.3rem 0.8rem;background:rgba(159,232,196,0.15);border:1px solid rgba(159,232,196,0.4);color:#9fe8c4;border-radius:6px;cursor:pointer;font-weight:600"
                :disabled="editItemsNewProductSaving || !editItemsNewProductForm.name.trim()"
                @click="saveEditItemsNewProduct">
                {{ editItemsNewProductSaving ? '...' : '✓ Створити та додати' }}
              </button>
              <button class="ghost-button" style="font-size:0.82rem" @click="showEditItemsNewProduct=false; editItemsNewProductError=''">Скасувати</button>
            </div>
          </div>

          <p v-if="editItemsError" class="error-text" style="font-size:0.82rem;margin-bottom:0.5rem">{{ editItemsError }}</p>

          <div style="display:flex;gap:0.5rem">
            <button
              style="font-size:0.85rem;padding:0.35rem 1rem;background:rgba(96,192,224,0.15);border:1px solid rgba(96,192,224,0.4);color:#60c0e0;border-radius:6px;cursor:pointer;font-weight:600"
              @click="saveEditOrderItems"
              :disabled="editItemsSaving || !editItemsList.length">
              {{ editItemsSaving ? 'Збереження...' : '✓ Зберегти зміни' }}
            </button>
            <button class="ghost-button" style="font-size:0.85rem" @click="showEditOrderItems=false">Скасувати</button>
          </div>
        </div>
      </div>
    </teleport>
