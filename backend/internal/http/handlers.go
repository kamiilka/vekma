...
type updateSupplierOrderRequest struct {
	...
	ClearCustomerOrderID bool   `json:"clearCustomerOrderId"`
...
func (s *Server) handleUpdateSupplierOrder(w http.ResponseWriter, r *http.Request) {
	...
	if req.ClearCustomerOrderID {
		if err := s.store.UpdateSupplierOrderCustomerLink(id, nil); err != nil {
			writeError(w, http.StatusBadRequest, err.Error())
			return
		}
	} else if req.CustomerOrderID != nil {
...
func (s *Server) handleUpdateSupplierOrderItems(w http.ResponseWriter, r *http.Request) {
	rawID := chi.URLParam(r, "id")
	id, err := strconv.ParseInt(rawID, 10, 64)
	if err != nil || id <= 0 {
		writeError(w, http.StatusBadRequest, "невірний ID замовлення постачальника")
	...
	if err := s.store.UpdateSupplierOrderItems(id, req.Items, req.Currency); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	s.store.AddAuditLog(
		userFromContext(r.Context()),
		"supplier_order.update_items",
		"supplier_order",
		fmt.Sprintf("supplier_order_id=%d items=%d", id, len(req.Items)),
	)
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok"})
}
