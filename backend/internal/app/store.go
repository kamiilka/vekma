...
func (s *Store) ListOrders() ([]CustomerOrder, error) {
	if s.db != nil {
		rows, err := s.db.Query(`
			SELECT
				co.id, co.customer_name, co.status, co.currency,
				co.total, co.total_uah, co.due_date, co.created_at, co.updated_at,
				COALESCE(SUM(p.amount) FILTER (WHERE p.currency = co.currency), 0) AS paid,
				COALESCE(SUM(p.amount_uah), 0) AS paid_uah
			FROM customer_orders co
			LEFT JOIN payments p ON p.order_id = co.id
			GROUP BY co.id
			ORDER BY co.id DESC
		`)
		if err != nil {
		...
		for rows.Next() {
			...
			order.Debt = order.Total - order.Paid
			if order.Debt < 0 {
				order.Debt = 0
			}
			order.DebtUAH = order.TotalUAH - order.PaidUAH
			if order.DebtUAH < 0 {
				order.DebtUAH = 0
			}
...
func (s *Store) refreshOrderPaymentStatusLocked(orderID int64) {
	...
	// Compute paid amount in order currency
	rateToUAH := 1.0
	if r, err := s.rateToUAHLocked(currency); err == nil && r > 0 {
		rateToUAH = r
	}
	paid := paidUAH / rateToUAH
	debtUAH := totalUAH - paidUAH
	...
	debt := debtUAH / rateToUAH

	for i := range s.orders {
		if s.orders[i].ID != orderID {
			continue
		}
		// Always update paid/debt fields regardless of status
		s.orders[i].Paid = paid
		s.orders[i].PaidUAH = paidUAH
		s.orders[i].Debt = debt
		s.orders[i].DebtUAH = debtUAH
		if nextStatus != "" && s.orders[i].Status != OrderStatusCancelled {
...
// UpdateSupplierOrderItems replaces all items of a supplier order (only for draft orders).
func (s *Store) UpdateSupplierOrderItems(supplierOrderID int64, items []PurchaseItem, currency string) error {
	if len(items) == 0 {
	...
	for _, item := range items {
		...
		if item.Price < 0 {
			return errors.New("item price must be >= 0")
		}
		total += float64(item.Quantity) * item.Price
	}
	...
	if s.db != nil {
		...
		var status string
		if err := tx.QueryRow(`SELECT status FROM supplier_orders WHERE id = $1`, supplierOrderID).Scan(&status); err != nil {
			return errors.New("supplier order not found")
		}
		if status != "draft" {
			return errors.New("can only edit items of draft orders")
		}
		...
		if _, err := tx.Exec(`DELETE FROM supplier_order_items WHERE supplier_order_id = $1`, supplierOrderID); err != nil {
			return err
		}
		...
		for _, item := range items {
			if _, err := tx.Exec(`
				INSERT INTO supplier_order_items (supplier_order_id, product_id, quantity, price)
				VALUES ($1, $2, $3, $4)
			`, supplierOrderID, item.ProductID, item.Quantity, item.Price); err != nil {
				return err
			}
		}

		// Update totals and currency
		if _, err := tx.Exec(`
			UPDATE supplier_orders SET total = $2, total_uah = $3, currency = $4, updated_at = NOW() WHERE id = $1
		`, supplierOrderID, total, totalUAH, currency); err != nil {
			return err
		}
	...
	for i := range s.supplierOrders {
		if s.supplierOrders[i].ID == supplierOrderID {
			if s.supplierOrders[i].Status != SupplierOrderStatusDraft {
				return errors.New("can only edit items of draft orders")
			}
			s.supplierOrders[i].Items = items
			s.supplierOrders[i].Total = total
			s.supplierOrders[i].Currency = currency
			s.supplierOrders[i].UpdatedAt = time.Now().UTC()
			return nil
		}
	}
